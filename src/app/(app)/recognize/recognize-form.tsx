...
type SimpleUser = { id: string; email: string; firstName?: string | null; lastName?: string | null };

export default function RecognizeForm({ users, available }: { users: SimpleUser[]; available: number }) {
  const [rows, setRows] = useState<{ userId: string; points: number }[]>([{ userId: users[0]?.id ?? "", points: 1 }]);
  ...
  function addRow() {
    setRows([...rows, { userId: users[0]?.id ?? "", points: 1 }]);
  }

  function removeRow(i: number) {
    const next = rows.slice();
    next.splice(i, 1);
    setRows(next.length ? next : [{ userId: users[0]?.id ?? "", points: 1 }]);
  }
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={addRow} className="text-blue-600 text-sm">+ Add recipient</button>
        </div>
        ...
        {rows.map((row, i) => (
          <div key={i} className="flex gap-2 items-center">
            ...
            />
            <button type="button" onClick={() => removeRow(i)} className="text-red-600 text-sm">Remove</button>
          </div>
        ))}
      </div>

      <div className="text-sm text-gray-700">Total: <b>{total}</b> / Available: <b>{available}</b></div>
