...
export async function POST(req: Request) {
  ...
  if (found.length !== ids.length) {
    return NextResponse.json({ error: "One or more recipients do not exist." }, { status: 400 });
  }
  ...
  if (total > available) {
    return NextResponse.json({ error: `Insufficient points. Available: ${available}`, available }, { status: 400 });
  }
